...
public class LoggingEventHandlerExecutor implements Executor
{
  ...
  public void execute(final Runnable command)
  {
    ...
    final LoggingEventHandlerThread runner = new LoggingEventHandlerThread(
      (LoggingEventHandler) command);
  ...
  /**
   * Attempts to perform a clean shutdown of all running {@link
   * LoggingEventHandler} tasks.
   */
  public void shutdown()
  {
    for (LoggingEventHandlerThread runner : runningThreads) {
      ...
      try {
        ...
        logger.warn(
          "Error on logging event handler shutdown: " + e.getMessage());
      }
    }
  }
  ...
  protected class LoggingEventHandlerThread extends Thread
  {
    ...
    public LoggingEventHandlerThread(final LoggingEventHandler handler)
    {
      super(
        handler,
        "gator-handler-log4j-" + handler.getRemoteAddress().getHostAddress());
