...
{
  protected static final Pattern PROJECT_NAME_PATTERN = Pattern.compile(
    "/project/([^/]+)/");

  protected static final Pattern APPENDER_ID_PATTERN = Pattern.compile(
    "/appender/(\\d+)/");

  protected static final Pattern CATEGORY_ID_PATTERN = Pattern.compile(
    "/category/(\\d+)/");

  protected static final Pattern CLIENT_ID_PATTERN = Pattern.compile(
    "/client/(\\d+)/");

  protected static final Pattern PERMISSION_ID_PATTERN = Pattern.compile(
    "/perm/(\\d+)/");
   ...
   *
   * @return  Project name in given request or null if no project name is found
   * in the URI.
   */
   ...
   *
   * @param  pattern  Pattern that contains a capture expression to extract the
   * parameter.
