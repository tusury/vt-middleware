...
public class CryptReader
{
    ...
    throws CryptException, IOException
  {
    final KeySpec spec;
    final String algorithm;
    final byte[] bytes = readData(keyStream);
    ...
    PrivateKeyInfo pi;
    try {
      pi = PrivateKeyInfo.getInstance(seq);
    } catch (Exception e) {
      pi = null;
    }
    if (pi != null) {
      ...
      // OpenSSL "traditional" format is essentially a sequence of a version
      // number and raw key parameters, e.g., for a DSA private key the sequence
      // {version, p, q, g, privExp}
      // where version is 0 for all practical purposes

      // Detect key type based on number of parameters:
      // RSA -> {version, mod, pubExp, privExp, prime1, prime2, exp1, exp2, c}
      // DSA -> {version, p, g, g, privExp}
      if (seq.size() == 9) {
        algorithm = "RSA";
        spec = new RSAPrivateCrtKeySpec(
            DERInteger.getInstance(seq.getObjectAt(1)).getValue(),
            DERInteger.getInstance(seq.getObjectAt(2)).getValue(),
            DERInteger.getInstance(seq.getObjectAt(3)).getValue(),
            DERInteger.getInstance(seq.getObjectAt(4)).getValue(),
            DERInteger.getInstance(seq.getObjectAt(5)).getValue(),
            DERInteger.getInstance(seq.getObjectAt(6)).getValue(),
            DERInteger.getInstance(seq.getObjectAt(7)).getValue(),
            DERInteger.getInstance(seq.getObjectAt(8)).getValue());
      } else if (seq.size() == 5) {
        algorithm = "DSA";
