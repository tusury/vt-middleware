 ...
 * file.  Provides a smaller memory footprint than {@link FilePointerWordList}
 * at the cost of performance.  Each word still must be read from the file for
 * every get, though the implementation supports a simple memory cache to
...
public class FileWordList extends AbstractWordList
{
   ...
   * <p>
   * <strong>NOTE</strong>
   * <p>  Attempts to close the source file will cause {@link IOException} when
   * {@link get(index)} is called subsequently.
   *
   * @param  raf  File containing words, one per line.
   * @param  caseSensitive  Set to true to create case-sensitive word list,
